...
test("normalized search, immutable snapshot, master changes, totals and PDF remain isolated", { skip: !databaseUrl }, async () => {
  ...
  try {
    ...
    assert.equal(client.filter((item) => item.pricingFamily === "ORDINARY").length, 124); assert.equal(vendor.filter((item) => item.pricingFamily === "ORDINARY").length, 118); assert.notEqual(client.length, vendor.length);
    const headcount = await db.commercialOffering.findFirstOrThrow({ where: { quantityBasis: "HEADCOUNT_DUTY", pricingFamily: null, durationPolicyId: null } });
    await assert.rejects(calculateNormalizedLine({ commercialOfferingId: headcount.id, configuration: { headcount: "1", dutyUnitsPerPerson: "1" } }, "TO_CLIENT", db), (error: unknown) => error instanceof NormalizedQuoteError && error.code === "OFFERING_NOT_PERSONNEL_READY");
    ...
    assert.equal(oldPlatform.usageDaysSnapshot, "4"); assert.equal(oldPlatform.chargeUnitsSnapshot, "2"); assert.equal(oldPlatform.billableQuantitySnapshot, "640"); assert.equal(oldPlatform.billableQuantityNumeratorSnapshot, "640"); assert.equal(chair.usageDaysSnapshot, "2"); assert.equal(chair.chargeUnitsSnapshot, "1"); assert.equal(carpet.chargeUnitsSnapshot, "1"); assert.equal(oldPlatform.rateScopeSnapshot, "GLOBAL"); assert.equal(oldPlatform.marketIdSnapshot, null); assert.ok(oldPlatform.priceIdSnapshot); assert.equal(oldPlatform.pricingEngineVersion, "quoteos-exact-v2");
