...
const schema = z.object({ code: z.string(), name: z.string().trim().min(2), mode: z.enum(["ONE_OFF", "USAGE_DAYS", "MANUAL"]), chargeMultiplierNumerator: z.number().int(), chargeMultiplierDenominator: z.number().int(), minimumChargeNumerator: z.number().int(), minimumChargeDenominator: z.number().int(), roundingMode: z.enum(["NONE", "CEIL", "FLOOR", "HALF_UP"]), description: z.string().nullable().optional() });
export async function GET() { const access = await requireRole("ADMIN"); if ("error" in access) return access.error; return NextResponse.json({ policies: await prisma.durationPolicy.findMany({ orderBy: [{ active: "desc" }, { code: "asc" }], include: { _count: { select: { offerings: true } } } }) }); }
