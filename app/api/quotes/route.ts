...
async function createLegacyQuote(input: z.infer<typeof legacyDraft>, actorId: string) {
  ...
  return prisma.$transaction(async (tx) => { const number = await allocateQuoteNumber(tx); const created = await tx.quote.create({ data: { number, type: input.type, company: input.company, project: input.project || null, venue: input.venue || null, city: input.city || null, salesperson: input.salesperson || null, createdById: actorId } }); const revision = await tx.quoteRevision.create({ data: { quoteId: created.id, revisionNumber: 1, status: "DRAFT", createdById: actorId, preparedDate: new Date(), taxPercentage: input.taxPercentage, quoteTypeSnapshot: input.type, companySnapshot: input.company, projectSnapshot: input.project || null, venueSnapshot: input.venue || null, citySnapshot: input.city || null, salespersonSnapshot: input.salesperson || null, currencySnapshot: "INR", termsSnapshot: "This quotation is subject to availability, final technical confirmation, and applicable taxes. Payment terms and project-specific conditions will be confirmed in writing.", settingsSnapshot: { builderMode: "LEGACY" }, ...totals, lines: { create: calculatedLines.map((line) => ({ catalogueItemId: line.catalogueItemId, itemCodeSnapshot: line.item.code, itemNameSnapshot: line.item.name, descriptionSnapshot: line.item.description, unitSnapshot: line.item.unit, quantity: line.quantity, days: line.days, rateUsedPaise: line.rateUsedPaise, discountPercent: line.discountPercent, discountPaise: line.discountPaise, lineTotalPaise: line.lineTotalPaise, remarks: line.remarks || null })) } }, include: { lines: true } }); await tx.quoteEvent.create({ data: { quoteId: created.id, actorId, action: "CREATED", metadata: { revisionId: revision.id, builderMode: "LEGACY" } } }); return { ...created, revision }; }, { isolationLevel: "Serializable" });
}
