...
export function AppSidebar({ isAdmin = false, isAccessManager = false }: { isAdmin?: boolean; isAccessManager?: boolean }) {
  ...
  const links = [...baseLinks,
    ...(access.isAdmin ? [{ href: "/admin/catalogue", label: "Catalogue & Rates", icon: "▦" }, { href: "/admin/packages", label: "Package recipes", icon: "◫" }, { href: "/admin/catalogue/review", label: "Catalogue Review / Release", icon: "✓" }, { href: "/catalogue", label: "Legacy inventory", icon: "◇" }] : []),
