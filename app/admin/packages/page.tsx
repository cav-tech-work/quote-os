...
export default async function PackageAdminPage() {
  ...
  return <main className="app"><AppSidebar isAdmin isAccessManager={user.accessManager} /><section className={styles.page}>
    ...
    {packages.length === 0 ? <section className={styles.empty}><h2>No package recipes yet</h2><p>No packages have been created yet. Start the first recipe manually.</p><AdminButton href="/admin/packages/new" variant="primary">New Package</AdminButton></section> : packages.map((pkg) => <article key={pkg.id} className={styles.card}>
