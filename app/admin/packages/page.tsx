...
export default async function PackageAdminPage() {
  ...
  return <main className="app"><AppSidebar isAdmin isAccessManager={user.accessManager} /><section className={styles.page}>
    <header className={styles.pageHeader}><div><small>COMMERCIAL COMPOSITIONS</small><h1>Package recipes</h1><p>Build draft recipes from catalogue components. Recipe quantity never represents stock.</p></div><AdminButton href="/admin/packages/new" variant="primary">Create Package</AdminButton></header>
    {packages.length === 0 ? <section className={styles.empty}><h2>No package recipes yet</h2><p>The CCI workbooks did not create packages. Start the first recipe manually.</p><AdminButton href="/admin/packages/new" variant="primary">New Package</AdminButton></section> : packages.map((pkg) => <article key={pkg.id} className={styles.card}>
