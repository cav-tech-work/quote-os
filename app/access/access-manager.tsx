...
export function AccessManager() {
  ...
  return <main className={styles.page}><header><div><p className="eyebrow">ADMINISTRATION</p><h1>QuoteOS access</h1></div><a href="/">Back to quotes</a></header><section><h2>Grant access</h2><p>Invite a user as a quote user or administrator. Access-manager authority cannot be delegated.</p><form onSubmit={submit}><input name="email" type="email" placeholder="name@clockwork-av.com" required /><select name="role" defaultValue="QUOTE_USER"><option value="QUOTE_USER">Quote user</option><option value="ADMIN">Administrator</option></select><button className="primary">Save access</button></form>{message && <p className={styles.message}>{message}</p>}</section><section><h2>Authorised users</h2><div className={styles.userList}>{users.map((user) => <div key={user.id}><span><b>{user.name ?? user.email}</b><small>{user.email}</small></span><span>{user.accessManager ? "Access manager" : user.role === "ADMIN" ? "Administrator" : "Quote user"}</span><span>{user.active ? "Active" : "Inactive"}</span></div>)}</div></section></main>;
}
