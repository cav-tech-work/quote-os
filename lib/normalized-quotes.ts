...
export const PRICING_ENGINE_VERSION = "quoteos-exact-v1";
export const SNAPSHOT_SCHEMA_VERSION = "normalized-line-v1";
...
export type NormalizedQuoteErrorCode = "OFFERING_NOT_FOUND" | "OFFERING_NOT_QUOTE_READY" | "RATE_UNAVAILABLE" | "DURATION_POLICY_UNAVAILABLE" | "CONFIGURATION_INVALID" | "CALCULATION_UNSUPPORTED" | "RATE_CONFLICT";
...
export async function listNormalizedOfferings(side: PriceSide, search = "", database: QuoteDatabase = prisma) {
  ...
  const offerings = await database.commercialOffering.findMany({
    where: {
      active: true,
      quantityBasis: { in: [...SUPPORTED_BASES] },
      OR: [{ kind: null }, { kind: { not: "PACKAGE" } }],
      durationPolicy: { is: { active: true, authority: "INTERNAL_APPROVED", mode: { not: "MANUAL" } } },
  ...
  return offerings.filter((offering) => offering.prices.length === 1).map((offering) => ({ id: offering.id, code: offering.code, name: offering.name, canonicalCode: offering.canonicalItem?.code ?? null, canonicalName: offering.canonicalItem?.name ?? null, quantityBasis: offering.quantityBasis, billingUnit: offering.billingUnit, durationPolicyCode: offering.durationPolicy!.code, durationPolicyMode: offering.durationPolicy!.mode, priceId: offering.prices[0].id, unitRatePaise: offering.prices[0].amountPaise, rateSide: side, rateScope: "GLOBAL" as const }));
}

export type NormalizedLineInput = { commercialOfferingId: string; configuration: MeasurementConfiguration; usageDays: string; overrideChargeUnits?: string; overrideReason?: string; expectedPriceId?: string };
...
function policyDefinition(policy: Awaited<ReturnType<typeof calculateNormalizedLine>>["policy"], usageDays: string | null) {
...
export function normalizedLineSnapshot(resolved: Awaited<ReturnType<typeof calculateNormalizedLine>>, input: NormalizedLineInput, discountPercent: number, remarks?: string) {
  ...
  return {
    ...
    days: calculation.usageDays ?? input.usageDays,
    ...
    normalizedConfigurationSnapshot: calculation.normalizedConfiguration as Prisma.InputJsonValue,
    quantityBasisSnapshot: calculation.quantityBasis!,
    ...
    usageDaysSnapshot: calculation.usageDays,
    durationPolicyIdSnapshot: policy.id,
    durationPolicyCodeSnapshot: policy.code,
    durationPolicyModeSnapshot: policy.mode,
    durationPolicyDefinitionSnapshot: policyDefinition(policy, calculation.usageDays),
    chargeUnitsSnapshot: calculation.chargeUnits!,
    durationResolutionProvenanceSnapshot: calculation.chargeUnitProvenance!,
    overrideChargeUnitsSnapshot: input.overrideChargeUnits ?? null,
    overrideReasonSnapshot: input.overrideReason?.trim() || null,
    baseAmountPaiseSnapshot: calculation.baseAmountPaise!,
    amountMeaningSnapshot: calculation.amountMeaning!,
...
export type NormalizedQuoteDraft = { type: "CLIENT" | "VENDOR"; company: string; project?: string; venue?: string; city?: string; salesperson?: string; taxPercentage: number; eventDays: number; lines: Array<Omit<NormalizedLineInput, "usageDays"> & { usageDays?: string; discountPercent: number; remarks?: string }> };
