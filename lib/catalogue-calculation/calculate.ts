...
export function calculateOfferingWithRate(offering: OfferingCalculationIdentity, inputConfiguration: MeasurementConfiguration, side: PriceSide, rate: GlobalRateResult, durationPolicy: DurationPolicyDefinition | null = null, durationInput: DurationInput = {}): OfferingPricingResult {
  ...
  const quantityExact = quantity.state === "QUANTITY_CALCULATED" ? { billableQuantityNumerator: quantity.exactNumerator, billableQuantityDenominator: quantity.exactDenominator } : { billableQuantityNumerator: null, billableQuantityDenominator: null };
  if (quantity.state !== "QUANTITY_CALCULATED") return { ...base, ...durationEmpty, ...quantityExact, normalizedConfiguration: null, billingUnit: offering.billingUnit, billableQuantity: null, priceId: null, unitRatePaise: null, baseAmountPaise: null, amountMeaning: null, pricingState: quantity.state, issues: quantity.issues };
  if (rate.state !== "RATE_FOUND") return { ...base, ...durationEmpty, ...quantityExact, normalizedConfiguration: quantity.normalizedConfiguration, billingUnit: quantity.billingUnit, billableQuantity: quantity.billableQuantity, priceId: null, unitRatePaise: null, baseAmountPaise: null, amountMeaning: null, pricingState: rate.state, issues: [] };
  ...
  if (offering.durationBasis === "DUTY") return { ...base, ...durationEmpty, ...quantityExact, normalizedConfiguration: quantity.normalizedConfiguration, billingUnit: quantity.billingUnit, billableQuantity: quantity.billableQuantity, priceId: rate.priceId, unitRatePaise: rate.amountPaise, baseAmountPaise: Number(amount), amountMeaning: "PER_CHARGE_PERIOD_BASE_AMOUNT", pricingState: "UNSUPPORTED_SPECIAL_DURATION", issues: [] };
  if (!durationPolicy) return { ...base, ...durationEmpty, ...quantityExact, normalizedConfiguration: quantity.normalizedConfiguration, billingUnit: quantity.billingUnit, billableQuantity: quantity.billableQuantity, priceId: rate.priceId, unitRatePaise: rate.amountPaise, baseAmountPaise: Number(amount), amountMeaning: "PER_CHARGE_PERIOD_BASE_AMOUNT", pricingState: "DURATION_POLICY_UNAVAILABLE", issues: [] };
  const duration = resolveChargeUnits(durationPolicy, durationInput);
  if (duration.state !== "CHARGE_UNITS_RESOLVED") return { ...base, ...durationEmpty, ...quantityExact, normalizedConfiguration: quantity.normalizedConfiguration, billingUnit: quantity.billingUnit, billableQuantity: quantity.billableQuantity, priceId: rate.priceId, unitRatePaise: rate.amountPaise, baseAmountPaise: Number(amount), amountMeaning: "PER_CHARGE_PERIOD_BASE_AMOUNT", pricingState: duration.state, issues: [duration.issue] };
  ...
  return { ...base, ...quantityExact, normalizedConfiguration: quantity.normalizedConfiguration, billingUnit: quantity.billingUnit, billableQuantity: quantity.billableQuantity, priceId: rate.priceId, unitRatePaise: rate.amountPaise, baseAmountPaise: Number(amount), durationPolicyId: duration.policyId, durationPolicyCode: duration.policyCode, usageDays: duration.usageDays, chargeUnits: duration.chargeUnits, chargeUnitProvenance: duration.provenance, finalAmountPaise: Number(final), amountMeaning: "FINAL_BASE_AMOUNT", pricingState: "READY", issues: [] };
}
